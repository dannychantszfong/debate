...
MODEL_PATH = r"G:\AI_models\gemma-4-E4B-it-heretic-Q4_K_M.gguf"
TOPIC_FULL = '''
...
- open_issues: 当前仍悬而未决的核心争点，仅描述争议本身，不给steering（列表，1-3条）
...
- positive（正方视角）：
  ...
  - recent_pressure_zone: 最近一两轮中，正方实际承受压力的争点区域（字符串，可为空，只能描述现状，不能指挥下一步）
- negative（反方视角）：（同上结构）
...
def normalize_debate_state(state: dict, known_unverified_refs=None) -> tuple[bool, dict | None, list]:
    ...
    normalized["shared_state"]["open_issues"] = _normalize_text_list(
    ...
    for side in ("positive", "negative"):
        ...
        normalized["side_state"][side]["recent_pressure_zone"] = _clean_state_text(
    ...
    for side in ("positive", "negative"):
        ...
        normalized["side_state"][side]["recent_pressure_zone"] = _scrub_reference_risks_from_text(
...
def _build_requirement_records(targets, state, rounds, current_turn, expires_after=2):
    ...
    anchor_parts = list(shared.get("open_issues", [])[:2])
    if rounds:
        recent_text = " ".join(
...
def _refresh_pending_requirements(moderator_state, state, rounds, current_turn):
    ...
    shared = state.get("shared_state", {}) if isinstance(state, dict) else {}
    anchor_text = " ".join(shared.get("open_issues", []))
    if rounds:
        recent_text = " ".join(
...
def filter_state_for_side(state, side, active_requirements=None):
    ...
    if "shared_state" in state and "side_state" in state:
        ...
        if own_d.get("recent_pressure_zone"):
            brief["你近期承受压力的争点区域"] = own_d["recent_pressure_zone"]
        ...
        if shared.get("open_issues"):
            brief["当前未决争点"] = shared["open_issues"]
...
def _state_marker_terms(state, side):
    ...
    text += " " + side_state.get("recent_pressure_zone", "")
...
def assess_round(rounds, moderator_state=None, window=TRIGGER_WINDOW):
    ...
    sev = round(min(1.0, base_sev + intervention_failure_score * 0.12), 3)
...
def main():
    ...
    log_path = os.path.join(LOG_DIR, f"debate_v{VERSION}_{title_slug}_{ts}.json")
    ...
    try:
        for turn in range(MAX_TURNS):
            ...
            # ── Positive speaks ──────────────────────────────────────────────────────
            if turn == 0:
                pos_input = mod_prefix + "请你作为正方，开场陈述你对该辩题的立场与核心论点。" + final_tail
            else:
            ...
            # Frame colonization check (advisory — logs warning, does not retry)
